...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled ? 'bg-background/95 backdrop-blur-md shadow-[var(--shadow-warm)]' : 'bg-transparent'
    }`}>
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navItems.map((item) => (
                <button
                  ...
                  className="text-foreground hover:text-primary px-3 py-2 text-sm font-medium transition-colors duration-200 hover:scale-105 transform"
                >
                  {item.label}
                </button>
              ))}
